...
create_vm_single(){
  ...
  qm create "$VMID" --memory 2048 --cores 2 --name "$NAME" --net0 virtio,bridge="$VMBR" --ide0 none || {
    log_err "创建虚拟机失败"; exit 1;
  }
  ...
  log_info "配置 SCSI 磁盘和开机顺序 (普通 VM)"
...
show_main_menu(){
  echo "================ PVE 一体化脚本 ================"
  echo "1) 创建单个云模板 (不写入 IP)"
  echo "2) 创建单个虚拟机 (DHCP 启动后写 IP 到 tags)"
